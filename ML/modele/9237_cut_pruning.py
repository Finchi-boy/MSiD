...
class Model_relu_sigmoid:
    ...
    def prune_weights(self, threshold: float) -> None:
        """
        Przycina wagi modelu, których wartość bezwzględna jest mniejsza od podanego progu.
        """
        total_weights = 0
        pruned_weights = 0

        print("\n--- ROZPOCZĘCIE PRUNINGU WAG ---")
        for i, layer in enumerate(self.all_layers):
            # Interesują nas tylko warstwy liniowe (Layer), pomijamy funkcje aktywacji
            if isinstance(layer, Layer):
                # Tworzymy maskę boolean dla wag, które są mniejsze od progu
                mask = np.abs(layer.weights) < threshold

                # Zerujemy wagi spełniające warunek maski
                layer.weights[mask] = 0.0

                # Zliczanie statystyk dla tej warstwy
                layer_total = layer.weights.size
                layer_pruned = np.sum(mask)

                total_weights += layer_total
                pruned_weights += layer_pruned

                print(f"Warstwa {i} ({layer.inputs_n} -> {layer.neurons_n}): "
                      f"Wyzerowano {layer_pruned}/{layer_total} wag ({layer_pruned/layer_total*100:.2f}%)")

        # Obliczenie całkowitej rzadkości (sparsity) modelu
        overall_sparsity = (pruned_weights / total_weights) * 100
        print(f"Globalna rzadkość modelu (Sparsity): {overall_sparsity:.2f}%\n")
...
if __name__ == "__main__":
    ...
    print("Wyniki przed pruningiem:")
    model.test(X_test, y_test, filepath="D:\\Users (D)\\amx10\\Desktop (D)\\msid\\MSiD\\ML\\new_model\\raport6.txt")

    model.prune_weights(threshold=0.05)

    print("Wyniki po pruningu:")
    model.test(X_test, y_test, filepath="D:\\Users (D)\\amx10\\Desktop (D)\\msid\\MSiD\\ML\\new_model\\raport6_pruned.txt")
